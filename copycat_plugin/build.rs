fn main() {
    ...
    if ok && dll_path.exists() {
        ...
        let dest = std::path::Path::new(&cargo_target).join(&profile).join("onnxruntime.dll");
...
fn download_curl(url: &str, dest: &std::path::Path) -> bool {
...
#[cfg(not(target_os = "windows"))]
fn extract_unzip(zip: &std::path::Path, out: &std::path::Path) -> bool {
...
fn download_powershell(url: &str, dest: &std::path::Path) -> bool {
...
#[cfg(target_os = "windows")]
fn extract_powershell(zip: &std::path::Path, out: &std::path::Path) -> bool {
    std::process::Command::new("powershell")
        .args(["-NoProfile", "-Command",
            &format!("Expand-Archive -Path '{}' -DestinationPath '{}' -Force", zip.display(), out.display())])
        .status().ok().map_or(false, |s| s.success())
}
